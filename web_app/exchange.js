...
const exchange_name = 'banana';             // TODO: fill in the name of your exchange

const token_name = 'banana';                // TODO: replace with name of your token
const token_symbol = 'BNN';              // TODO: replace with symbol for your token
...
const token_address = '0xF6733AB90988c457a5Ac360D7f8dfB9E24aA108F';      
const token_abi = [
  {
    "inputs": [],
...
const exchange_address = '0x455DE73b2Ddf008E53b4D274A0F0b065bB3C7023';                
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    // let price = await exchange_contract.methods.priceTokenPerETH().call(); 
    // let amountToken = Math.ceil(parseInt(amountEth * price) / 10 ** 18).toString();
    // token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, "500000"); // Max number of tokens in the pool. TODO: make it dynamic instead
    var state = await getPoolState();
    amountToken = Math.ceil(state.token_eth_rate * amountEth);
    var max_exchange_rate = state.token_eth_rate * (100 + maxSlippagePct);
    max_exchange_rate = Math.ceil(10**8 * max_exchange_rate);
    var min_exchange_rate = state.token_eth_rate * (100 - maxSlippagePct);
    min_exchange_rate = Math.ceil(10**8 * min_exchange_rate);
    await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountToken);
    return exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity(max_exchange_rate, min_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), "wei")});
}
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    ...
    var min_exchange_rate = state.token_eth_rate * (100 - maxSlippagePct);
    min_exchange_rate = Math.ceil(10**8 * min_exchange_rate);
    return exchange_contract.connect(provider.getSigner(defaultAccount)).removeLiquidity(amountEth, max_exchange_rate, min_exchange_rate);
}
...
async function removeAllLiquidity(maxSlippagePct) {
    ...
    var min_exchange_rate = state.token_eth_rate * (100 - maxSlippagePct);
    min_exchange_rate = Math.ceil(10**8 * min_exchange_rate);
    return exchange_contract.connect(provider.getSigner(defaultAccount)).removeAllLiquidity(max_exchange_rate, min_exchange_rate);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    ...
    var state = await getPoolState();
    var max_exchange_rate = state.token_eth_rate * (100 + maxSlippagePct);
    max_exchange_rate = Math.ceil(10**8 * max_exchange_rate); // TODO floor?

    await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountToken);
    return exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(amountToken, max_exchange_rate);
}

async function swapETHForTokens(amountEth, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    var state = await getPoolState();
    var max_exchange_rate = state.eth_token_rate * (100 + maxSlippagePct);
    max_exchange_rate = Math.ceil(10**8 * max_exchange_rate)
    return exchange_contract.connect(provider.getSigner(defaultAccount)).swapETHForTokens(max_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), "wei")});
}
